...
When trained on an English corpus a Word2Vec model can be tested using similies from WordNet. It's possible to test for syntactic similarity ("read is to reading as walk is to walking") and semantic similarity ("Berlin is to Germany as Paris is to France").
